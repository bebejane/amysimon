...
import s from './ArchiveGallery.module.scss';
...
export default function ArchiveGallery({ collections }: Props) {
	...
	const handleNext = () => {
		...
		const idx = atDescription
			? collection.artwork.length
			: index[collection.id] >= slideCount - 1
				? 0
				: index[collection.id] + 1;
	...
	const handleZoomIn = async ({ target }) => {
		...
		if (!isMobile) {
			try {
				...
				const dImage = document
					.getElementById(`slides-${id}`)
					.querySelector<HTMLImageElement>(`figure:nth-of-type(${idx[collection.id] + 1}) picture>img`);

				setShowCollection(true);

				const caption = document.getElementById(id).querySelector<HTMLElement>('figcaption>span');
				const dCaption = document
					.getElementById(`caption-${id}-${idx[id]}`)
					.querySelector<HTMLElement>('span:nth-child(1)');
				const dCaptionText = document
					.getElementById(`caption-${id}-${idx[id]}`)
					.querySelector<HTMLElement>('span:nth-child(2)');
		...
		if (!isMobile && !isTextSlide) {
			try {
				...
				const image = document
					.getElementById(`slides-${collection.id}`)
					.querySelector<HTMLImageElement>(`figure:nth-of-type(${idx + 1}) picture>img`);
				...
				if (image && dImage && !isTextSlide) {
					const clone = await transitionImage(
						image,
						dImage,
						transitionDuration,
					...
					clone.remove();
				} else await sleep(transitionDuration);
			} catch (err) {
				console.error(err);
			}
		}
	...
	useEffect(() => {
		if (hoverCollectionId) setCollection(collections.find(({ id }) => id === hoverCollectionId));
	}, [hoverCollectionId]);
		...
		<>
			...
			{collections.map((c, idx) => (
				<div
					id={`gallery-${c.id}`}
					key={c.id}
					className={cn(s.gallery, showCollection && collection?.id === c.id && s.visible)}
				>
					<header className={cn(s.desktop, fullscreen && s.fullscreen)}>
						<span id={`gallery-year-${c.id}`} className={cn(s.year, 'track')}>
							{c?.year ? `${c.year}${c.yearEnd ? `–${c.yearEnd}` : ''}` : 'Also'}
						</span>
						<span className={s.close} onClick={handleZoomOut}>
							Close
						</span>
					</header>
					...
					<div id={`slides-${c.id}`} className={cn(s.slides, isMobile && collection?.id !== c.id && s.hide)}>
						{c.artwork.map((artwork, i) => (
							<figure
								key={artwork.id}
								className={cn(
									((i === index[c.id] && collection?.id === c.id && showCollection) || isMobile) && s.show,
									s[artwork.layout]
								)}
							>
								{artwork.image?.responsiveImage && (
									<Image
										...
										className={cn(
											s.image,
											allThumbsLoaded && (i === 0 || collection?.id === c.id) && s.load,
											videoPlayId === artwork.id && s.hide
										)}
										fadeInDuration={0}
										usePlaceholder={true}
										placeholderClassName={s.placeholder}
									...
									<div className={s.video}>
										{videoPlayId === artwork.id ? (
											<div className={s.wrapper} id={`video-${artwork.id}`}>
												<div className={s.close} onClick={() => setVideoPlayId(null)}>
													Back
												</div>
												<Youtube
					...
					{c.artwork.length > 1 && index[c.id] < c.artwork.length && (
						<div className={cn(s.indicators)}>
							{c?.artwork.map((el, i) => (
								<span key={i} className={cn(i === index[c.id] && s.active)}>
									•
								</span>
							))}
						</div>
					...
					{(c.artwork.length > 1 || (c.description && c.artwork.length === 1)) &&
						collection?.id === c.id &&
						showCollection &&
						!transitioning && <GalleryNav show={true} onNext={handleNext} onPrev={handlePrev} />}
				</div>
			))}
		</>
	);
...
export const transitionImage = async (
	...
	clone.style.transition = ['top', 'left', 'width', 'height', 'opacity']
		.map((prop) => `${prop} ${easing} ${dur}ms`)
		.join(',');
