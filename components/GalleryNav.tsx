...
export default function GalleryNav({ show: showFromProps = true, className, onPrev, onNext }: GalleryNavProps) {
	...
	const handleMouseEnter = ({ target }: React.MouseEvent<HTMLSpanElement>) =>
		setShow((target as HTMLElement).id as 'prev' | 'next' | null);
	...
	return (
		<nav className={cn(s.nav, className)}>
			<span
				id='prev'
				className={cn(s.prev, show === 'prev' && s.show)}
				onClick={onPrev}
			...
			<span
				id='next'
				className={cn(s.next, show === 'next' && s.show)}
				onClick={onNext}
				onMouseMove={handleMouseMove}
				onMouseEnter={handleMouseEnter}
				onMouseLeave={handleMouseLeave}
